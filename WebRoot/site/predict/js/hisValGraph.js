...
$(function() {
	...
	$("#query_lstzz").click(function() {
		...
		var wl_class = $("#wl_sel").val();
		$.getJSON('getLSTZZ_RES', {
			table : table,
			instr_no : instr_no,
			wl_class : wl_class
...
function drawPic(data, id,comp_index,rise) {
	...
	title = $("#point_sel").val().split('#')[1] + ' ' + the_comp + ' ' + rise;
	var ranges = [];
	var averages = [];
	$.each(data, function(index, element) {
		ranges.push([ Date.parse(element.year), Number(element.minV),
				Number(element.maxV) ]);
		averages.push([ Date.parse(element.year), Number(element.avgV) ]);
	});
