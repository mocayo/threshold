...
//	point.attr('multiple','multiple');
	$.getJSON('getObservePoints',{basicId:basicId,monitorItem:monitorItem,instr_type:instr_type},function(result){
		...
		$.each(result,function(index,element){
			var tmp = element.split("#");
			if(index==0)
				option += '<option selected value="' + $.trim(tmp[1])+ '#' + $.trim(tmp[0]) + '">' + $.trim(tmp[0]) + '</option>';
			else
...
//	});
	var _point = point.val().split('#')[1];
	if(_point == '')return;
//	if(the_point=='')return;
//	console.log(the_point);
//	comp.attr('multiple','multiple');
	$.getJSON('getComponent',{point:/*points.toString()*/_point},function(result){
		console.log(result);
//		var option = '<option value="" selected>---请选择---</option>';
		var option = '';
		$.each(result,function(index,element){
