...
public class DataPredictAutomaticAutoMI {
	...
	public DataPredictAutomaticAutoMI(IMonitorItemTypeDao monitorItemTypeDao,
			...
			IMidCharacteristicDao midCharacterDao,ILstzzParasDao lstzzParasDao ) {
	...
	public void jobTimer() {
		...
		for (int d = 0; d < 100; d++) {
			calendar.set(Calendar.DAY_OF_MONTH, calendar
					.get(Calendar.DAY_OF_MONTH) + 1);// 让日期加1
...
//			String tableName = "T_ZB_PL";
			...
			/*
			 * if (iswhereList.size() == 0) continue;
			 */
				...
				try {
					...
					if (msR1Str.equals("") && msR2Str.equals("")
						...
						method4(map, dt, tableName, id, anlys2, expFit, poly2Fit,
								"r1,r2,r3");
//						method5(map, dt, tableName, id, anlys2, expFit, poly2Fit,
//								"r1,r2,r3");
						} else {
						for (int j = 0; j < msR1.length; j++) {
							...
							if (!methodName.equals("")) {
								try {
									Method m = DataPredictAutomaticAutoMI.class
											.getDeclaredMethod(methodName,
													...
													poly2FitModel.class,
													String.class);
									m.invoke(new DataPredictAutomaticAutoMI(
											monitorItemTypeDao, definssortDao,
											commonPredictDao, sin_preAutoDao,
											dataPredictedDao, commonPreHandleDao,
											modelPointDao,midCharacterDao,lstzzParasDao), map, dt, tableName, id,
			...
			method6(tableName,dt);
			map.put("tableName", tableName);
			map.put("dt", dt);
			try {
				commonPredictDao.updRes(map);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
...
//		dataPredictedDao.updPredicted(map);
	...
	private void method7(Map<String, Object> map, String dt, String tableName,
			String id, BestInterp useless1, expFitModel useless2,
			poly2FitModel anlys2, String component) {
		Map<String, Object> resultMap = null;
...
//		dataPredictedDao.updPredicted(map);
	...
	private void method5(Map<String, Object> map, String dt, String tableName,
		...
		if (date.after(nextDate)) {
			...
			MidCharacteristic midTemp = null;
			try {
				midTemp = midList.get(0);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				continue;
	 ...
	 * @param tableName
	 */
	private void method6(String tableName,String dt){
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("tableName",tableName);
		map.put("dt",dt);
