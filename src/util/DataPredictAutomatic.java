...
public class DataPredictAutomatic extends Action {
	...
	public void autoPreprocessByDay() throws Exception{
...
*/
	...
	public void workstateTest() throws Exception{
		String day_str = "2008-08-08";
		//添加预处理工作状态 -- 0:正在预处理
		...
		//预测
		//添加预测工作状态 -- 0:正在预测
		System.out.println("添加预测工作状态 -- 0:正在预测");
	...
	public void executeByDay() throws IOException {
		long startTimeMillis = System.currentTimeMillis() / 1000L;
//		System.setOut(new PrintStream("D://threshold-logs//executeByDay-stdout-log-2.log"));
		// Date today = new Date();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		...
		Calendar end = Calendar.getInstance();
		try {
			start.setTime(sdf.parse(currentDay));
			end.setTime(sdf.parse(day));
			start.set(Calendar.DAY_OF_MONTH, start.get(Calendar.DAY_OF_MONTH) + 1);
			currentDay = sdf.format(start.getTime());
			System.out.println("开始日期:" + currentDay + " 结束日期:" + day + "......");
			//为了让end也参与计算 end + 1，因为before方法类似小于
//			end.set(Calendar.DAY_OF_MONTH, end.get(Calendar.DAY_OF_MONTH) + 1);
			while(start.compareTo(end) < 1){
				String day_str = sdf.format(start.getTime());
				
				System.out.println("开始计算" + day_str + "的数据。。。");
				...
				for(String tableName:tableNameList){
					map.put("tableName", tableName.trim());
					System.out.println("修改" + map.get("dt") + " of " + tableName.trim());
					commonPredictDao.addRes_auto(map);//修改isBest标志位
				}
...
//				sendPost("http://127.0.0.1:8090/WebServiceEva.asmx/ComEva","");
		...
		/*try {
//			String str = "2016-04-09";
			String str = day;
			Date today = sdf.parse(str);
			System.out.println("开始计算" + str + "的数据。。。");
			
			PreHandleAutomatic pre = new PreHandleAutomatic(commonPreHandleDao,monitorItemTypeDao,
							modelPointDao,dataCollectionDao); 
			pre.preHandleAll(str);
			
			jobTimer(sdf, str, today);
			...
			map.put("dt",sdf.format(next.getTime()));
			System.out.println("the dt is " + map.get("dt") + " || " + sdf.format(next.getTime()));
			for(String tableName:tableNameList){
				map.put("tableName", tableName.trim());
				System.out.println("修改" + map.get("dt") + " of " + tableName.trim());
				commonPredictDao.addRes_auto(map);//修改isBest标志位
			...
			Calendar tmpCalendar = Calendar.getInstance();
			tmpCalendar.setTime(sdf.parse(day));
			tmpCalendar.set(Calendar.DAY_OF_MONTH, tmpCalendar.get(Calendar.DAY_OF_MONTH) - 1);
			map.put("dt",sdf.format(tmpCalendar.getTime()));
			System.out.println("修改日期为 " + map.get("dt"));
			System.out.println("开始做sigama评判" + day + ".....");
			for (String tableName : tableNameList) {
				map.put("tableName", tableName.trim());
				try {
					ceNodeMetMemDao.addlixue12(map);
					ceNodeMetMemDao.addlixue3(map);
					method6(tableName.trim(),tmpCalendar.getTime(),next, sdf);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
			System.out.println("sigama评判" + map.get("dt") + "结束.....");
			System.out.println("current || temp || log 表更新开始。。。。。");
			try {
				ceNodeMetMemDao.reSetCurrent();
				ceNodeMetMemDao.addCurrent();
				ceNodeMetMemDao.updLog(str);
			} catch (Exception e) {
				e.printStackTrace();
			}
			System.out.println("current || temp || log 表更新结束。。。。。");
			System.out.println(str + "阈值计算结束。。。。");
			System.out.println("开始调用昆明院综合评判");
			sendPost("http://127.0.0.1:8090/WebServiceEva.asmx/ComEva","");
		} catch (Exception e) {
		...
		*/
		...
		System.out.println("计算结束,共需时间:"+(System.currentTimeMillis() / 1000L - startTimeMillis));
...
//		String day = request.getParameter("day");
		...
		try {
			...
			///Query the state of DatePrdiction of the dayh
			workstateMap.remove("state");
			workstateMap.remove("name");
			workstateMap.put("state", "0");
			workstateMap.put("name", "datacalculate");
			int countPreState0 = sin_preAutoDao.queryDataPreState(workstateMap);
			workstateMap.remove("state");
			workstateMap.put("state", "1");
			int countPreState1 = sin_preAutoDao.queryDataPreState(workstateMap);
			...
			Calendar next = Calendar.getInstance();
			next.setTime(sdf.parse(day_str));
			next.set(Calendar.DAY_OF_MONTH, next.get(Calendar.DAY_OF_MONTH) + 1);
			String nextStr = sdf.format(next.getTime());
			...
			workstateMap.remove("name");
			workstateMap.remove("state");
			workstateMap.remove("caijiTime");
			workstateMap.put("name", "datacalculate");
			workstateMap.put("state", "0");
			workstateMap.put("caijiTime", nextStr);
			sin_preAutoDao.insertWorkState(workstateMap);

			jobTimer(sdf, day_str, start.getTime());
			...
			map.put("dt",nextStr);
			System.out.println("the dt is " + map.get("dt") + " || " + nextStr);
			for(String tableName:tableNameList){
				map.put("tableName", tableName.trim());
				System.out.println("修改" + map.get("dt") + " of " + tableName.trim());
				commonPredictDao.addRes_auto(map);//修改isBest标志位
...
//			String str = "2016-04-09";
			...
			commonPredictDao.addCurrentTDay(day);
		} catch (Exception e) {
			System.out.println("TAT...。。。。。。。。。。。。。。。。。。。。。。。。。。。。。");
			e.printStackTrace();
		}
	...
	public DataPredictAutomatic(IMonitorItemTypeDao monitorItemTypeDao, IDEFINSSORTDao definssortDao,
	...
	public DataPredictAutomatic(IMonitorItemTypeDao monitorItemTypeDao, IDEFINSSORTDao definssortDao,
			...
			ICommonPreHandleDao commonPreHandleDao, IModelPointDao modelPointDao, IMidCharacteristicDao midCharacterDao,
			ILstzzParasDao lstzzParasDao, IW_CENodeMetMemDao ceNodeMetMemDao, ISigamaDao sigamaDao,
			IDataCollectionDao dataCollectionDao,IT_Water_LevelDao t_water_levelDao, int da) {
		this.monitorItemTypeDao = monitorItemTypeDao;
		this.definssortDao = definssortDao;
		this.commonPredictDao = commonPredictDao;
		this.sin_preAutoDao = sin_preAutoDao;
	...
	public void jobTimer(SimpleDateFormat sdf, String str, Date date) throws Exception {
		...
		try {
			t_water_levelDao.convertWL(str);
		} catch (Exception e2) {
			e2.printStackTrace();
		}
		try {
			ceNodeMetMemDao.addLog(str);
		} catch (Exception e2) {
			e2.printStackTrace();
		}
		try {
		...
		String updTime = sf.format(tmpDate);
		...
		System.out.println("ADD PREDICT STATE END!");
		
		map.clear();
		map.put("stateDate", str);
		map.put("stateName", "judgement");
		map.put("stateStr", "0");
		map.put("countCaculated", countCaculated);
		try {
			dataCollectionDao.addWorkState(map);
		...
		System.out.println("ADD JUDGE STATE END!");
	...
	private int method2(Map<String, Object> map, Date date, Calendar calendar, SimpleDateFormat sdf, String tableName,
		...
		if ((itemType.getIsR1() == 1) && (liR1 == null || liR1.size() == 0)) {
			...
			if (resultMap == null) {
				System.out.println("结果map为空！......................");
			} else {
				System.out.println("结果map不为空！");
...
//							e.printStackTrace();
	...
	private void method6(String tableName, Date date, Calendar calendar, SimpleDateFormat sdf) {
		System.out.println("method6   sigama判定");
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("tableName", tableName);
		calendar.setTime(date);
		long sTime = System.nanoTime() / 1000000L;
...
//				e.printStackTrace();
	...
	public int method7(Map<String, Object> map, String tableName, String id, Date date, Calendar calendar,
		...
		if (((itemType.getIsR1() == 1) && (liR1 != null && liR1.size() == 0))
				&& ((itemType.getIsR2() == 1) && (liR2 != null && liR2.size() == 0))) {
			try {
